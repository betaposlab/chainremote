...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    try {
      ...
      try {
        // SYSTEM user failed
        // macOS 에선 app_dir 를 비워둬 Rust 가 네이티브 config 경로
        ...
        //    대상이라 config(영구비번·id)가 기기 간 덮어써진다.
        //  - getApplicationSupportDirectory() 로 바꿔도 APP_DIR 은 Flutter init(늦은 시점)에서만
        //    설정되어, core_main(이른 시점)은 Preferences, 이후는 ApplicationSupport 로 경로가
        //    갈려 영구비번이 startup 에 유실된다(단계 간 config 분리).
        //  - app_dir 를 비우면 이른·늦은 시점 모두 Rust ProjectDirs 경로(Preferences, iCloud 무관)로
        //    통일된다.
        //  - Windows 서비스도 SYSTEM 에서 이 호출이 실패해 빈 app_dir 로 동일하게 동작한다(정상).
        if (!isMacOS) {
