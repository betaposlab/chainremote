...
2. **재성이 PC가 1.4.4로 잘 업데이트됐는지 눈으로 확인** — 재성이는 본사용(HQ) 앱이라 서버 DB로 버전 추적이 안 됨. 재성이 앱이 1.4.4 되고 로그인·원격이 정상인지 **직접 봐야** 함. 혹시 업데이트하다 앱이 망가지면(설치 실패) → RDP(크롬 원격)로 들어가 수동 재설치. (자동업데이트가 과거에 앱을 망가뜨린 적 있음 — 메모리 project_updater_brick_incident.)
