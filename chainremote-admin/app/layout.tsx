...
export const metadata: Metadata = {
  ...
  //
  // 검색 노출은 랜딩(/main/)만 원한다. 패널은 전부 로그인 뒤의 화면이고, 유일하게
  //   공개인 로그인 화면이 "체인리모트" 검색 결과에 소개 페이지 대신 뜨면 곤란하다.
  //
  // ★follow 는 켜 둔다(noindex, follow). 이 화면을 색인하진 말되 **링크는 따라가라**는
  //   뜻이다 — 로그인 화면의 [제품 소개 보기] 가 /main/ 으로 가는 링크이고, 지금 그
  //   페이지로 들어오는 외부 링크가 하나도 없다(구글 URL 검사: "참조 페이지 없음").
  //   nofollow 로 막으면 스스로 만든 유일한 발견 경로를 닫는 셈이다.
  //
  // ★robots.txt 로 Disallow 하지 않는 이유: 크롤러가 아예 안 들어오면 이 noindex 를
  //   읽지도 못해서, 오히려 색인이 안 지워진다. 막으려면 들여보내고 말해야 한다.
