    ...
    <div className="px-8 py-6 max-w-7xl">
      <header className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-[#8a93ad] mt-1">{rows.length}건 표시</p>
        </div>
      </header>
      ...
      <form className="mb-4 flex items-center gap-2 text-sm">
        <select
          name="period"
          defaultValue={period}
          className="rounded-md border border-[#172540] px-2 py-1.5"
        >
        ...
        <button
          type="submit"
          className="rounded-md bg-white/[0.06] hover:bg-white/[0.1] px-3 py-1.5 text-[#c7c9d1]"
        >
          필터
        </button>
        {(periodParam || customerId) && (
          <Link href="/sessions" className="text-xs text-[#6b7390] hover:text-[#abaebb] underline">
            초기화
          </Link>
        )}
      </form>
