...
//   deviceId 가 오면(=새 앱) active_login_sessions 에서 좌석 1개 확보 시도.
//   다른 기기가 점유 중이면 409 → 앱이 "강제 종료하고 사용 / 취소" 모달 → takeover.
//   deviceId 미전송(=옛 앱)은 enforcement 없이 그대로 발급 → 백워드 호환(§8).
...
export async function POST(req: Request) {
  try {
    ...
    // 좌석 확보 — device_id 가 있는 새 앱만. 옛 앱(미전송)은 active 행 없이 그대로 발급.
    if (deviceId) {
      ...
      if (!claim.claimed) {
        // 409 OCCUPIED — 다른 기기에서 사용 중. 토큰 발급 안 함.
    ...
    const { token, expiresIn } = await signApiToken(
      ...
      // 좌석 확보된 새 앱만 jti 박음. 옛 앱은 jti 없이(= 옛 동작 그대로).
      deviceId ? jti : undefined,
    );
