...
const inp =
  "rounded-md border border-[#7485ae] px-3 py-2 text-sm focus:border-[#4c7dff] focus:outline-none";
...
function Group({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#566999] bg-[#3d4e7a]">
      <div className="flex items-center justify-between px-5 py-4">
        <button
          onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-3 text-left"
        >
          <span className="w-4 text-[#ccd2e3]">{open ? "▾" : "▸"}</span>
          <span className="font-semibold text-white">
            {company.displayName}
          </span>
          <span className="rounded bg-white/[0.06] px-2 py-0.5 text-xs text-[#cbd1e0]">
            아이디 {company.users.length}개
          </span>
        </button>
        <Link
          href={`/admin/tenants/${company.id}/edit`}
          className="text-xs text-[#ccd2e3] hover:text-[#c3d3ff]"
        >
          회사 정보 →
        </Link>
      </div>
      {open && (
        <div className="border-t border-[#51638f] bg-white/[0.02] p-4">
...
function UserTable({
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#566999] bg-[#3d4e7a]">
      <table className="panel-table">
        ...
        <tbody>
          {users.length === 0 && (
            <tr>
              <td colSpan={7} className="px-3 py-4 text-center text-[#ccd2e3]">
                아이디 없음 — 위에서 추가하세요.
              </td>
            </tr>
          )}
  ...
  return (
    <>
      <tr className="hover:bg-white/[0.04]">
        ...
        <td className="px-3 py-2 text-[#cbd1e0]">@{user.email}</td>
        ...
        <td className="whitespace-nowrap px-3 py-2 text-right">
          {isSelf ? (
            <span className="text-xs text-[#ccd2e3]">—</span>
          ) : (
            <>
              <button
