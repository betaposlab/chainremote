...
function SortHeader({
  ...
  return (
    <th className={`text-left px-4 py-3 font-medium whitespace-nowrap ${className ?? ""}`}>
      ...
      >
        {label}
        <span className="text-[10px] text-[#ccd2e3]">
      ...
      <div className="panel-table-wrap overflow-x-auto">
        <table className="panel-table">
          ...
          <tbody>
                ...
                <tr key={c.id} data-search={searchHay}>
                  <td className="px-4 py-3">
                    <Link
                      ...
                      className="font-medium hover:text-[#c3d3ff]"
                    >
                      {c.name}
                    </Link>
                      ...
                      <span
                        className={
                          c.assignedUserId === currentUserId
                            ? "inline-block bg-[#4c7dff]/15 text-[#c3d3ff] px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap"
                            : "text-[#cbd1e0] whitespace-nowrap"
                        }
                      >
                        {c.assignedUserName}
                      </span>
                  ...
                  <td className="px-4 py-3 font-mono text-xs hidden md:table-cell">
                    {c.remoteId ? (
                      ...
                      <span className="text-[#ccd2e3]">미등록</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    {c.remoteId ? (
                      ...
                      <Link
                        href={`/customers/${c.id}/edit`}
                        className="inline-flex items-center text-xs text-[#ccd2e3] hover:text-[#c3d3ff] px-3 py-1.5"
                      >
                        ID 등록
                      </Link>
                    )}
                    ...
                    <Link
                      href={`/customers/${c.id}/edit`}
                      className="inline-flex items-center text-xs text-[#b9bfd2] hover:text-white px-2 py-1.5 ml-1"
                    >
                      수정
                    </Link>
                  </td>
            ...
            <tr id="cust-search-empty" style={{ display: "none" }}>
              <td colSpan={8} className="px-4 py-12 text-center text-[#ccd2e3] text-sm">
                검색 결과가 없습니다.
              </td>
            </tr>
          </tbody>
        ...
        {rows.length === 0 && (
          <div className="px-4 py-12 text-center text-[#ccd2e3] text-sm">
            아직 등록된 거래처가 없습니다.
            <Link href="/customers/new" className="text-[#c3d3ff] hover:underline ml-2">
              첫 거래처 추가하기
            </Link>
          </div>
        )}
