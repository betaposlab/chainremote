...
/**
 * 폴더 이름 변경 — 자기 tenant 폴더만. 같은 tenant 에 그 이름이 이미 있으면 'dup'(uq_folders_tenant_name).
 * 소속 거래처의 device_group_name 은 folder join 으로 유도되므로 rename 만으로 HQ 표시가 따라온다.
 */
export async function renameFolder(
  id: string,
  name: string,
  tenantId: string,
): Promise<"ok" | "notfound" | "dup"> {
  const trimmed = name.trim();
  if (!trimmed) throw new Error("폴더 이름이 필요합니다");
  // 다른 폴더가 그 이름을 이미 쓰면 중복(자기 자신으로의 rename 은 허용).
  const [dup] = await db
    .select({ id: folders.id })
    .from(folders)
    .where(and(eq(folders.tenantId, tenantId), eq(folders.name, trimmed)))
    .limit(1);
  if (dup && dup.id !== id) return "dup";
  const [row] = await db
    .update(folders)
    .set({ name: trimmed })
    .where(and(eq(folders.id, id), eq(folders.tenantId, tenantId)))
    .returning({ id: folders.id });
  return row ? "ok" : "notfound";
}
