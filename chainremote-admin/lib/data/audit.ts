...
export type AuditAction =
  ...
  // 거래처 목록에 없는 ID 로 원격을 건 것. 정상 세션은 support_sessions 에 남으므로
  //   감사에 또 담지 않는다(그러면 감사 화면이 세션 목록이 되고 삭제 한 줄이 묻힌다).
  //   목록 밖 접속만 남긴다 — "직원이 우리 거래처가 아닌 어딘가에 붙었다"는 그쪽에서만
  //   확인할 수 있고, 지금은 skipped 로 조용히 넘어가 아무 데도 안 남는다.
  | "session.unknown_peer"
  | "push.bulk"
