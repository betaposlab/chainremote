...
#   - Python (res/inline-sciter.py 실행용)
#
# ⚠ Write-Host 메시지는 ASCII only. powershell -File 은 한국어 윈도우에서 파일을 CP949 로 읽어
#   비ASCII 문자열 리터럴이 깨지면 따옴표 균형이 무너져 ParserError 가 난다 (2026-06-10 두 번 발생).
#   주석(# 뒤)의 한글은 파싱에 무관하므로 유지. 진단 출력만 영어.
#
...
Write-Host '[1/6] check sciter.dll (x86)'
...
# ★ 핵심: inline feature 의 Sciter UI(cm.tis/index.tis 등)는 res/inline-sciter.py 가
#   src/ui/inline.rs(.gitignore, untracked 생성물) 로 구워넣는다. 이 단계를 빠뜨리면
#   옛 inline.rs(이전 빌드 잔재)로 조용히 빌드돼 UI 변경이 반영 안 됨(거짓 OK).
Write-Host '[3/6] regen Sciter inline.rs (bake cm.tis/index.tis changes)'
...
Write-Host '[4/6] clean libsodium-sys + i686 build (inline sciter, no hwcodec)'
...
Write-Host '[5/6] PE check (machine=0x014C + subsystem min) + smoke'
...
Write-Host '[6/6] stage payload (agent32-payload = x86 side of unified installer)'
