...
# SYSTEM 권한 예약작업(ChainRemoteServiceWatchdog)이 10분마다 실행한다.
#
# ⚠ PS 2.0 호환 (Win7 32비트 거래처 기본 PowerShell): `*> $null`(PS3+)과
#   `Get-ChildItem -Directory`(PS3+) 금지. PS5 에서도 동일 동작 — x64/x86 공용 단일 파일.
#
# 배경 (Phase 3-Win 이후 변화):
#   v1.2.19 부터 트레이 "서비스 중지" 메뉴가 영구 숨김돼, 영구 stop-service 토글로 인한
#   hbbs 미등록 사고는 더 이상 없다. 이제 이 워치독은 안전망 차원에서 아주 가볍게 ChainRemote.toml 만 손본다.
#
#   남겨둘 가치가 있는 안전망:
#     - 서비스가 어쩌다 죽은 채 방치되는 케이스 (Windows sc failure 미설정).
#     - 누가 sc delete ChainRemote 로 서비스 자체를 날린 케이스 → 재생성.
#     - heartbeat cold-boot race — 가동 중 재시작은 원격 끊김 위험이라
#       이 워치독은 "재시작" 은 안 한다 (인스톨러 [Run] 1회 + 다음 재부팅에 위임).
#
# 매 틱 동작:
#   1) ChainRemote.toml/ChainRemote2.toml 에 stop-service=Y 라인이 있으면 제거.
#   2) 서비스가 없으면(sc delete 당했으면) 재생성 + 시작. 시작유형 auto 보장.
#   3) 서비스가 Stopped 면 시작. 정상 가동 중이면 무기록 종료 — 로그 스팸 방지.
...
if ($null -eq $svc) {
    ...
    if ($null -eq $exe) { return }   # ChainRemote 미설치(파일 없음) — 관여 안 한다
...
elseif ($svc.Status -ne 'Running') {
    ...
    # 정상 가동 중 — 아무것도 안 남기고 종료 (가동 중 재시작은 원격세션 끊김 위험)
    return
}
