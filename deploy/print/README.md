# 인쇄물 (우편 발송·계약용)

- `pamphlet.html` — 대리점 안내서 **정본.** 고칠 때는 여기를 고친다.
- `체인리모트_대리점_안내서.pdf` — 인쇄소에 넘기거나 메일에 붙이는 파일. HTML 에서 뽑는다.
- `이용신청서.html` / `체인리모트_이용신청서.pdf` — A4 한 장짜리 계약 서식. 두 부 뽑아
  한 부씩 나눠 갖는다. 약관 전문은 종이에 안 싣고 `626.kr/main/terms.html` 로 넘긴다 —
  27개 조를 봉투에 같이 넣을 이유가 없다.
- `인쇄견적요청서.html` / `체인리모트_인쇄견적요청서.pdf` — 인쇄소에 안내서 PDF 와 함께
  보내는 A4 한 장. 수량별(250/500/1,000) 빈 표를 채워 돌려받아 나란히 비교한다.
  ★사양은 **A3 양면 4도 → 중앙 반접 → A4 4면**, 스노우지 180g/m². 블리드는 없다
  (사방 15mm 여백이라 가장자리까지 차는 색이 없다).
- `체인리모트_이용약관.pdf` — **정본은 여기가 아니라** `deploy/web/chainremote/terms.html`
  이다. 그 파일에 인쇄용 CSS(`@media print`)가 들어 있어 그대로 뽑으면 A4 7쪽이 나온다.
  약관을 고쳤으면 이 PDF 도 다시 뽑을 것:

  ```
  "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
    --headless=new --disable-gpu --virtual-time-budget=9000 --no-pdf-header-footer \
    --print-to-pdf="$PWD/deploy/print/체인리모트_이용약관.pdf" \
    "file://$PWD/deploy/web/chainremote/terms.html"
  ```

★**사업자 주체가 바뀌면 고칠 곳은 네 군데**다(문성텔레콤 → 새 법인): 이 폴더의
`이용신청서.html` 공급자 상자, 그리고 `deploy/web/chainremote/` 의 `terms.html`(서문과
맨 끝 두 곳) · `privacy.html` · `index.html` 푸터. 네 파일 모두 그 자리에 주석을 달아 뒀다.

## PDF 다시 뽑기

안내서. 신청서도 파일 이름만 바꿔 같은 명령으로 뽑는다.

```
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  --headless=new --disable-gpu --virtual-time-budget=8000 --no-pdf-header-footer \
  --print-to-pdf="$PWD/deploy/print/체인리모트_대리점_안내서.pdf" \
  "file://$PWD/deploy/print/pamphlet.html"
```

브라우저 인쇄와 같은 엔진이라 A4 나눔과 인쇄용 CSS 가 그대로 간다.
`--virtual-time-budget` 은 웹폰트와 QR(스크립트로 그린다)이 그려질 시간을 준다 — 짧으면
글꼴이 대체 폰트로, QR 이 빈 칸으로 나간다. **뽑은 뒤에는 쪽수·크기·그림을 반드시 확인할 것**:

```
mdls -name kMDItemNumberOfPages -name kMDItemPageWidth 체인리모트_대리점_안내서.pdf
   → 4쪽 / 594.96pt(= A4 210mm) 이어야 정상
pdfimages -list 체인리모트_대리점_안내서.pdf
   → 1쪽 로고·수락카드·QR / 2쪽 배너 / 4쪽 아바타 그림자 조각(300dpi)
     (3쪽 타임라인과 4쪽 HQ 목업은 벡터라 여기 안 나오는 게 정상이고 인쇄 품질에 더 좋다)
```

## 판형

A4 4면. 양면 2장으로 접거나 A3 반접지로 뽑으면 봉투 한 통에 들어간다.
`.sheet` 는 높이가 고정이고 `overflow:hidden` 이라 **넘치면 조용히 잘린다.** 내용을 늘렸으면
브라우저에서 `scrollHeight - clientHeight` 가 네 면 모두 0 인지 확인하고 넘길 것.

## 그림 해상도

화면 캡처라 원본 해상도가 정해져 있다. 인쇄 권장은 300 DPI 인데, 표시 폭에 따라 이렇게 된다.

| 그림 | 실효 DPI |
|---|---|
| 수락 카드(1쪽) | 142 — 표지의 주인공이라 크기를 유지했다 |
| 원격 중 배너(2쪽) | 148 — 폭을 줄여 올렸다 |
| 거래처 목록(4쪽) | 벡터 — 캡처가 아니다 |

더 올리려면 VM 에서 더 큰 화면으로 다시 찍는 수밖에 없다. 실물로 뽑아 보고 판단할 것.

## 4쪽 거래처 목록은 왜 캡처가 아닌가

대리점이 실제로 쓰는 화면은 관리 패널이 아니라 **본사 앱(HQ)** 이다. 그런데 HQ 를 그대로
찍어 이 폭에 넣으면 140dpi 남짓이라 배지 글자가 뭉개지고, 캡처에는 실제 거래처 상호가
딸려 나온다. 그래서 HQ 라이트 팔레트(`CrColors.light`)와 카드 구조(`peer_card.dart`)를
그대로 옮겨 **HTML 로 그린다** — 인쇄에서 또렷하고, 상호는 예시로 고른다.
HQ 카드 모양이 바뀌면 `.hqm` 블록도 같이 손볼 것.
