...
# ── [0.7/5] 인스톨러 .iss 중괄호 수지 ───────────────────────────────────────
# ★2026-08-20 실측: 두 인스톨러에서 12줄이 `{{ ... }}` 로 적혀 있었다. Inno 는 `{` 만
#   겹쳐 쓰고 `}` 는 안 겹치므로 닫는 괄호가 하나 더 나가 PowerShell 이 파싱에서 죽는다 —
#   그 [Run] 항목이 통째로 실행되지 않는다. 서비스 중지·시작 같은 건 코어 --silent-install 이
#   대신 해줘서 몇 달간 안 보였고, 대신해 줄 게 없던 SELFTEST 만 "로그 0건"으로 드러났다.
#   사람 눈에는 짝이 맞아 보이니 기계가 센다.
echo "[0.7/5] 인스톨러 중괄호 수지 확인..."
python3 "$REPO/deploy/win-installer/check-braces.py" || exit 1
...
if [[ "$KIND" == "agent" ]]; then
  ...
  NAS_HOST=chang@100.93.42.91 bash "$REPO/deploy/publish/publish-hq.sh" "$DEST" "$NOTES"
  ...
  if ! bash "$REPO/deploy/publish/publish-landing.sh" "$REPO/dist/ChainGo_v$VERSION.exe"; then
    echo "⚠ 랜딩 배포 실패 — HQ 발행 자체는 완료. deploy/publish/publish-landing.sh 단독 재실행 필요." >&2
  fi
fi
