...
/// 걸려 있는 예약 — **아직 시작 전이어도** 돌려준다. 만료됐으면 None(마커도 치운다).
///
/// ★`is_open()` 과 다른 질문에 답한다. 그쪽은 "지금 들어와도 되나"라는 **보안 판정**이라
/// 시작 시각 전에는 반드시 거짓이어야 한다. 반면 "예약이 걸려 있나"는 화면이 묻는 질문이다.
/// 둘을 같은 함수로 답했더니 **시작 전 예약이 어느 화면에도 안 보였다** — 트레이에도,
/// 패널에도, 본사 메뉴에도. 그래서 걸어 놓고 시작 전에 무르려 해도 취소할 길이 없었다
/// (2026-08-18 Chang: 22:45 시작 예약을 22:43 에 취소하려는데 메뉴가 안 나왔다).
///
/// 여기를 넓혀도 접근 권한은 한 톨도 안 넓어진다. 통과 판정은 `is_open()` 하나뿐이고
/// 그건 그대로다.
pub fn status() -> Option<SchedWindow> {
    let w = read()?;
    let t = now();
    // 끝난 창은 없는 것이다. 여기서 치워야 만료된 마커가 남아 다음 판정을 헷갈리지 않는다.
    if t >= w.end {
        clear();
        return None;
    }
    // 승인 후 24시간 절대 상한 — 시계가 뒤로 튄 경우의 안전망(is_open 과 같은 이유).
    if t - w.granted_at >= MAX_WINDOW_SECS {
        clear();
        return None;
    }
...
/// 지금 이 순간 실제로 통과되는 구간인가 — 화면 문구를 "예약됨"과 "허용 중"으로 가른다.
pub fn is_active_now() -> bool {
    status().map(|w| now() >= w.start).unwrap_or(false)
}
