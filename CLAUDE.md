...
**고객이 할 일**: ① 받은 .exe 더블클릭(최초 1회 설치) ② 본사가 원격 시도하면 뜨는 **'수락' 버튼 클릭**. 끝.
- 코이노 4단계(988.co.kr 방문→세션입력→다운→실행) 대비 대폭 축소 = 핵심 차별화.
- **접속 정책 = 클릭 수락**(`approve-mode=click`). 거래처가 매 세션 '수락'을 눌러야 원격 시작 — 거래처 통제권 + 보안. **영구비번/무인 0클릭 모드는 안 씀** (2026-06-01 Chang 확정).
- UX 단순화를 깨는 제안 거부.
